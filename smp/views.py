...
from smp.models import Paper, Auther, Jounery, Prize, zzauthor, zhuanzhu, prauthor, zhuanli, zlauthor, middlezl, \
	middlepr, middlezz
...
def sta(req):
	...
	return render_to_response("static.html", {"jnum": j.count(), "anum": a.count(), "pnum": pnum, 'user': 'hehe',
	                                          'categories': categories, 'data': data, 'ans': ans, "ans1": ans1,
	                                          "ans2": ans2, "ans3": ans3, "point": point, "point1": point1,
	                                          "point2": point2, "point3": point3, "points": points, "sum1": sum1,
...
def Price(request):
	...
	if request.POST:
		...
		if not flag:
			...
			z.save()
			status = True
			return HttpResponseRedirect("/prize/viewprize/")
...
def changeprize(req):
	...
	if (req.method == "GET"):
		if (req.GET.get('id', False)):
			...
			content = {'level_list': level_list, 'prize_list': prize_list, "prize": prize, 'name': json.dumps(name),
			           'age': json.dumps(age)}
...
def changezl(req):
	if (req.method == "GET"):
		...
		z.save()
		return HttpResponseRedirect('/patent/viewzl/')
...
def changezz(req):
	if (req.method == "GET"):
		...
		z.gaintime = post['gaintime']
		z.save()
		return HttpResponseRedirect('/book/viewzz/')
